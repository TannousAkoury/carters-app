...
export default function Home({sessionUser,permissions}:{sessionUser:{id:string;email:string;role:string};permissions:string[]}) {
  ...
  const navigate=useCallback((target:View)=>{if(can(target))setView(target)},[can]);
  ...
  const openOrders = (filter: OrderFilter = "all") => { if(!can("orders"))return;setOrderFilter(filter); setView("orders"); };

  if (!ready) return null;
  return (
    <div className={`${styles.shell} ${styles.professionalShell}`}>
      <aside className={`${styles.sidebar} ${styles.professionalSidebar}`}>
      ...
      {commandOpen&&<div className={styles.commandBackdrop} role="presentation" onMouseDown={()=>setCommandOpen(false)}><section className={styles.commandPalette} role="dialog" aria-modal="true" aria-label="Admin navigation" onMouseDown={event=>event.stopPropagation()}><header><span>⌕</span><input autoFocus value={commandQuery} onChange={event=>setCommandQuery(event.target.value)} onKeyDown={event=>{if(event.key==="Enter"&&commandItems[0])chooseCommand(commandItems[0].view)}} placeholder="Search pages and tools…" aria-label="Search pages and tools"/><kbd>Esc</kbd></header><div className={styles.commandResults}>{commandItems.length?commandItems.map(item=><button key={item.view} onClick={()=>chooseCommand(item.view)}><span className={styles.commandItemIcon}>{item.icon}</span><span className={styles.commandItemCopy}><strong>{item.label}</strong><small>{item.description}</small></span><span className={styles.commandItemGroup}>{item.group}</span></button>):<div className={styles.commandEmpty}><span>⌕</span><strong>No matching tools</strong><small>Try a page name such as orders, editor, or settings.</small></div>}</div><footer><span>Navigate with search</span><span><kbd>Enter</kbd> open &nbsp; <kbd>Esc</kbd> close</span></footer></section></div>}
    </div>
  );
}

function Nav({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: string; label: string }) {
  return <button className={`${styles.navItem} ${active ? styles.navActive : ""}`} onClick={onClick}><span>{icon}</span>{label}</button>;
}
...
function Dashboard({ setView, openOrders, publishedAt, can, userName }: { setView: (v: View) => void; openOrders: (filter: OrderFilter) => void; publishedAt: string; can:(view:View)=>boolean; userName:string }) {
  ...
  const metrics = [
    {value:summary?.sessions ?? "—",label:"App sessions",note:`Selected ${selectedDays}-day period`,icon:"↗",tone:styles.dashboardIconBlue},
  ...
  return <div className={`${styles.content} ${styles.dashboardPage}`}>
    <section className={styles.dashboardWelcome}><div className={styles.dashboardWelcomeCopy}><div className={styles.dashboardDate}>{new Intl.DateTimeFormat(undefined,{weekday:"long",month:"long",day:"numeric"}).format(new Date())}</div><h2>Good to see you, {userName}.</h2><p>Dashboard reporting period: {selectedPeriod}</p></div><div className={styles.dashboardHeroActions}><AnalyticsDateRangePicker value={range} loading={loading} align="left" onApply={next=>setRange(next)}/><div className={styles.dashboardHeroButtons}><button className={styles.secondary} disabled={loading} onClick={()=>void refresh()}>{loading?"Refreshing…":"↻ Refresh data"}</button>{can("editor")&&<button className={styles.primary} onClick={()=>setView("editor")}>Open app editor</button>}</div></div></section>
    ...
    <section className={styles.dashboardMainGrid}>
      <article className={`${styles.dashboardCard} ${styles.dashboardChartCard}`}><header className={styles.dashboardCardHeader}><div><h3>App engagement</h3><p>Recorded sessions · {selectedPeriod}</p></div>{can("analytics")&&<button onClick={()=>setView("analytics")}>View analytics →</button>}</header><div className={styles.dashboardChartSummary}><div><strong>{summary?.sessions??"—"}</strong><span>App sessions · {selectedDays} days</span></div><div><strong>{summary?.screenViews??"—"}</strong><span>Screen views</span></div><div><strong>{summary?.productViews??"—"}</strong><span>Product views</span></div></div><div className={styles.dashboardChart}>{(summary?.days??[]).length?(summary?.days??[]).map((day,index)=><div key={`${day.label}-${index}`} className={styles.dashboardBarColumn}><div><span style={{height:`${Math.max(5,(day.value/maxDay)*100)}%`}}/></div><small>{day.label}</small></div>):<div className={styles.dashboardEmpty}>{loading?"Loading engagement…":"No recorded sessions yet"}</div>}</div></article>
    ...
    ].filter(([, , ,target])=>can(target as View)).map(([icon,title,copy,target])=><button className={styles.dashboardQuickAction} key={title} onClick={()=>setView(target as View)}><span>{icon}</span><div><strong>{title}</strong><small>{copy}</small></div><b>→</b></button>)}</section>
