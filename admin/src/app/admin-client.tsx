...
function Analytics(){
  ...
  const [analytics,setAnalytics]=useState<AppAnalytics|null>(null);const [range,setRange]=useState<AnalyticsDateRange>(initialRange);const [loading,setLoading]=useState(true);const [message,setMessage]=useState("");
  useEffect(()=>{let active=true;const params=new URLSearchParams(range);fetch(`/api/analytics/summary?${params}`,{cache:"no-store"}).then(async response=>{const data=await response.json();if(!response.ok)throw new Error(data.error||"App analytics unavailable.");if(active)setAnalytics(data)}).catch(error=>{if(active)setMessage(error instanceof Error?error.message:"App analytics unavailable.")}).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[range]);
  const applyRange=(next:AnalyticsDateRange)=>{setLoading(true);setMessage("");setRange(next)};
  ...
  return <div className={`${styles.content} ${styles.analyticsPage}`}>
    <section className={styles.analyticsHero}><div><p>APP ANALYTICS</p><h2>Mobile app performance</h2><span>A Shopify-style overview of activity inside your Carter&apos;s mobile app.</span></div><div className={styles.analyticsHeroActions}><AnalyticsDateRangePicker value={range} loading={loading} onApply={applyRange}/><i className={`${styles.tag} ${message?styles.tagGray:styles.tagGreen}`}>{loading?"Loading":message?"Unavailable":"Live app events"}</i></div></section>
