...
type StaffUser = { id: string; email: string; role: string; createdAt: string; updatedAt: string };
...
function Team(){
  ...
  const addEmployee=async()=>{setStaffMessage("Saving employee...");try{const response=await fetch("/api/admin-users",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:employeeEmail,role:employeeRole,password:employeePassword})});const data=await response.json();if(!response.ok)throw new Error(data.error||"Unable to add employee.");setEmployeeEmail("");setEmployeePassword("");setShowEmployeeForm(false);setStaffMessage("Employee can now sign in with their email and password.");await loadStaff()}catch(error){setStaffMessage(error instanceof Error?error.message:"Unable to add employee.")}};
  ...
  const resetEmployee=async(user:StaffUser)=>{const password=window.prompt(`New password for ${user.email}`);if(password===null)return;setStaffMessage("Resetting password...");try{const response=await fetch(`/api/admin-users/${user.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({password})});const data=await response.json();if(!response.ok)throw new Error(data.error||"Unable to reset password.");setStaffMessage("Password reset. Share the new password with the employee securely.");await loadStaff()}catch(error){setStaffMessage(error instanceof Error?error.message:"Unable to reset password.")}};
  const rolesWithMembers=roles.map(role=>({...role,members:role.name==="Owner"?1:staffUsers.filter(user=>user.role===role.name).length}));
  return <div className={styles.content}><div className={styles.tabBar}><button className={tab==="staff"?styles.tabActive:""} onClick={()=>setTab("staff")}>Staff & roles</button><button className={tab==="activity"?styles.tabActive:""} onClick={()=>setTab("activity")}>Activity log</button></div>{tab==="staff"?<div className={styles.teamGrid}><section className={styles.card}><div className={styles.cardHead}><div><h2>Admin team</h2><p>Add employees with email, role, and password access.</p></div><button className={styles.primary} onClick={()=>setShowEmployeeForm(true)}>Add employee</button></div>{showEmployeeForm&&<div className={styles.roleForm}><label>Employee email<input value={employeeEmail} onChange={e=>setEmployeeEmail(e.target.value)} placeholder="employee@company.com"/></label><label>Role<select className={styles.selectField} value={employeeRole} onChange={e=>setEmployeeRole(e.target.value)}>{roles.map(role=><option key={role.id} value={role.name}>{role.name}</option>)}</select></label><label>Temporary password<input value={employeePassword} onChange={e=>setEmployeePassword(e.target.value)} type="password" placeholder="At least 8 characters"/></label><div><button className={styles.secondary} onClick={()=>setShowEmployeeForm(false)}>Cancel</button><button className={styles.primary} onClick={addEmployee}>Create employee</button></div></div>}{staffMessage&&<div className={styles.staffNotice}>{staffMessage}</div>}<div className={styles.staffList}><div><span className={styles.avatar}>SA</span><div><strong>Store admin</strong><small>Default local administrator</small></div><i className={`${styles.tag} ${styles.tagGreen}`}>Owner</i><small>Full workspace access</small><button>⋯</button></div>{staffUsers.map(user=><div key={user.id}><span className={styles.avatar}>{initials(user.email)}</span><div><strong>{user.email}</strong><small>Added {new Date(user.createdAt).toLocaleDateString()}</small></div><i className={`${styles.tag} ${styles.tagBlue}`}>{user.role}</i><small>Employee login</small><div className={styles.staffActions}><button onClick={()=>resetEmployee(user)}>Reset</button><button onClick={()=>deleteEmployee(user)}>Delete</button></div></div>)}</div></section><section className={styles.card}><div className={styles.cardHead}><div><h2>Roles</h2><p>Create role templates for staff accounts.</p></div><button className={styles.primary} onClick={()=>setShowRoleForm(true)}>Add role</button></div>{showRoleForm&&<div className={styles.roleForm}><label>Role name<input value={roleName} onChange={e=>setRoleName(e.target.value)} placeholder="Content manager"/></label><label>Access scope<input value={roleScope} onChange={e=>setRoleScope(e.target.value)} placeholder="Content and campaigns"/></label><label>Description<textarea rows={3} value={roleDescription} onChange={e=>setRoleDescription(e.target.value)} placeholder="What can this role do?"/></label><div><button className={styles.secondary} onClick={()=>setShowRoleForm(false)}>Cancel</button><button className={styles.primary} onClick={addRole}>Save role</button></div></div>}<div className={styles.roleList}>{rolesWithMembers.map(role=><article key={role.id}><div><strong>{role.name}</strong><small>{role.scope}</small><p>{role.description}</p></div><span>{role.members} member{role.members===1?"":"s"}</span></article>)}</div></section></div>:<section className={styles.card}><div className={styles.cardHead}><div><h2>Activity log</h2><p>No persistent audit database is connected.</p></div></div><div className={styles.timeline}>{["Admin login configured","Homepage editor available","Push campaign tracking enabled"].map((item)=><div key={item}><span>✓</span><div><strong>{item}</strong><small>Local workspace event</small></div></div>)}</div></section>}</div>
}
