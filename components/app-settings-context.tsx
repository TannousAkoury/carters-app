...
export function AppGate({ children }: { children: ReactNode }) {
  ...
  return <View style={styles.gate}>
    <View style={styles.badge}><Text style={styles.badgeText}>{maintenance ? "MAINTENANCE" : "UPDATE REQUIRED"}</Text></View>
    <Text style={styles.logo}>Carter&apos;s</Text>
    <Text style={styles.title}>{maintenance ? "We’ll be right back" : "Please update the app"}</Text>
    <Text style={styles.message}>{maintenance ? "We’re making a few improvements to your shopping experience. Please try again shortly." : settings.updateMessage}</Text>
    {!maintenance && settings.updateUrl ? <TouchableOpacity style={styles.primary} onPress={openUpdate}><Text style={styles.primaryText}>Update now</Text></TouchableOpacity> : null}
    <TouchableOpacity style={styles.secondary} onPress={refresh}><Text style={styles.secondaryText}>Check again</Text></TouchableOpacity>
    {!maintenance ? <Text style={styles.version}>Installed {currentVersion} · Required {settings.minimumVersion}</Text> : null}
  </View>;
}

const styles = StyleSheet.create({
  gate: { flex: 1, backgroundColor: "#fffaf6", alignItems: "center", justifyContent: "center", padding: 32 }, badge: { backgroundColor: "#eaf3f8", paddingHorizontal: 13, paddingVertical: 7, borderRadius: 18, marginBottom: 20 }, badgeText: { color: "#397ab5", fontSize: 11, fontWeight: "900", letterSpacing: 1 }, logo: { color: "#397ab5", fontSize: 30, fontWeight: "900", marginBottom: 28 }, title: { color: "#002041", fontSize: 25, fontWeight: "900", textAlign: "center" }, message: { color: "#657083", fontSize: 15, lineHeight: 23, textAlign: "center", maxWidth: 340, marginTop: 12, marginBottom: 26 }, primary: { width: "100%", maxWidth: 320, height: 52, borderRadius: 26, backgroundColor: "#002041", alignItems: "center", justifyContent: "center" }, primaryText: { color: "#fff", fontWeight: "900" }, secondary: { padding: 16, marginTop: 4 }, secondaryText: { color: "#397ab5", fontWeight: "900" }, version: { color: "#9a8f8b", fontSize: 11, marginTop: 8 },
});
