...
export function GlobalNotificationButton() {
  ...
  return <TouchableOpacity style={[styles.button, (pathname === "/" || pathname === "/index") && styles.homeButton]} onPress={() => router.push("/notifications")} accessibilityLabel={`Notifications, ${unread} unread`}><Ionicons name="notifications-outline" size={22} color="#397ab5" />{unread > 0 && <View style={styles.badge}><Text style={styles.badgeText}>{unread > 9 ? "9+" : unread}</Text></View>}</TouchableOpacity>;
}
