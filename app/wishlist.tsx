  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCopy}><Text style={styles.title}>My Wishlist</Text><Text style={styles.count}>{items.length} saved {items.length === 1 ? "item" : "items"}</Text></View>
        {items.length ? <TouchableOpacity onPress={clear} style={styles.clearButton}><Text style={styles.clearText}>Clear</Text></TouchableOpacity> : <View style={styles.iconButton} />}
      </View>
          ...
          renderItem={({ item }) => {
            ...
            return <TouchableOpacity style={styles.card} activeOpacity={0.86} onPress={() => router.push({ pathname: "/product/[handle]", params: { handle: item.handle } })}>
              ...
              <View style={styles.details}>
                ...
                <Text style={styles.open}>View product</Text>
              </View>
              <TouchableOpacity onPress={() => remove(item.id)} style={styles.remove} accessibilityLabel={`Remove ${item.title} from wishlist`}>
                <Ionicons name="heart" size={22} color="#e0938e" />
              </TouchableOpacity>
            </TouchableOpacity>;
          }}
          ListEmptyComponent={<View style={styles.empty}><View style={styles.emptyIcon}><Ionicons name="heart-outline" size={38} color="#397ab5" /></View><Text style={styles.emptyTitle}>Your wishlist is empty</Text><Text style={styles.emptyText}>Tap the heart on products you love and they will appear here.</Text><TouchableOpacity style={styles.shopButton} onPress={() => router.replace("/")}><Text style={styles.shopButtonText}>Start shopping</Text></TouchableOpacity></View>}
        />
      )}
    </SafeAreaView>
  );
