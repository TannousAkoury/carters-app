...
export default function CartScreen() {
  ...
  const loadCart = useCallback(async () => {
    try {
      ...
      if(nextCart&&pendingReward&&!existingReward){
        try{if(!customerToken)throw new Error(t("cart.rewardSignIn"));nextCart=await attachCustomerToShopifyCart(nextCart.id,customerToken)||nextCart;nextCart=await applyShopifyCartDiscount(nextCart.id,pendingReward)}catch(reason){setError(reason instanceof Error?reason.message:t("cart.rewardApplyError"))}
      }
    ...
    } catch(reason) { setError(reason instanceof Error?reason.message:t("cart.loadError")); }
  ...
  const changeQuantity = async (lineId: string, quantity: number) => {
    ...
    catch { setError(t("cart.updateError")); }
    finally { setUpdating(''); }
  };
  ...
  const checkout = async () => {
    ...
    } catch (reason) { setError(reason instanceof Error?reason.message:t("cart.checkoutError")); }
    finally { setLoading(false); }
  };
...
const styles = StyleSheet.create({
  ...
  arabicParagraph: { writingDirection: 'rtl', textAlign: 'right' },
