...
export default function ProductScreen() {
  ...
  useEffect(() => {
    ...
    }).catch(() => mounted && setError(t("product.loadError"))).finally(() => mounted && setLoading(false));
    return () => { mounted = false; };
  }, [handle, requestedSize, locale, t]);
  ...
  const buyNow = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("product.checkoutError"));
  ...
  const addToCart = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("product.cartError"));
    } finally { setAdding(false); }
  };

  if (loading) return <View style={styles.center}><ActivityIndicator size="large" color="#06A2E4" /></View>;
  if (!product) return <View style={styles.center}><Text style={styles.error}>{error || t("product.notFound")}</Text></View>;
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Text style={[styles.title, hasArabicText(product.title) && styles.arabicText]}>{product.title}</Text>
        ...
        <View style={styles.sectionHeading}><Text style={styles.sectionTitle}>{hasSizes ? t("product.selectSize") : t("product.selectOption")}</Text>{hasSizes ? <TouchableOpacity onPress={() => setShowSizeChart(true)}><Text style={styles.sizeChartLink}>{t("product.sizeChart")}</Text></TouchableOpacity> : null}</View>
        ...
        {giftOptions ? <View style={styles.giftSection}>
          <Text style={styles.giftSectionTitle}>{t("product.giftPackaging")} <Text style={styles.optional}>({t("common.optional")})</Text></Text>
          <View style={styles.giftChoices}>
            <TouchableOpacity onPress={() => setGiftChoice(giftChoice === "wrap" ? "none" : "wrap")} style={[styles.giftCard, giftChoice === "wrap" && styles.giftCardSelected]} accessibilityRole="radio" accessibilityState={{ checked: giftChoice === "wrap" }}>
              ...
              <View style={styles.giftCardCopy}><Text style={styles.giftTitle}>{t("product.giftWrap")}</Text><Text style={styles.giftPrice}>{t("common.free")}</Text></View>
            ...
            <TouchableOpacity onPress={() => setGiftChoice(giftChoice === "box" ? "none" : "box")} style={[styles.giftCard, giftChoice === "box" && styles.giftCardSelected]} accessibilityRole="radio" accessibilityState={{ checked: giftChoice === "box" }}>
              ...
              <View style={styles.giftCardCopy}><Text style={styles.giftTitle}>{t("product.giftBox")}</Text><Text style={styles.giftPrice}>{formatMoney(giftOptions.boxPrice)}</Text></View>
              <Ionicons name={giftChoice === "box" ? "radio-button-on" : "radio-button-off"} size={21} color="#002041" />
            </TouchableOpacity>
          </View>
        </View> : null}
        {product.description ? <Text style={[styles.description, hasArabicText(product.description) && styles.arabicParagraph]}>{product.description}</Text> : null}
      ...
      <View style={styles.footer}>
        <TouchableOpacity disabled={!selected || adding} onPress={addToCart} style={[styles.addCart, (!selected || adding) && styles.buyDisabled]}>
          {adding ? <ActivityIndicator color="#002041" /> : <Text style={styles.addCartText}>{added ? `${t("product.added")} ✓` : t("product.addToCart")}</Text>}
        </TouchableOpacity>
        <TouchableOpacity disabled={!selected || buying} onPress={buyNow} style={[styles.buy, (!selected || buying) && styles.buyDisabled]}>
          {buying ? <ActivityIndicator color="#fff" /> : <Text style={styles.buyText}>{t("product.buyNow", { price: formatMoney(selected?.money) })}</Text>}
        </TouchableOpacity>
      </View>
      <Modal visible={showSizeChart} transparent animationType="slide" onRequestClose={() => setShowSizeChart(false)}>
        <View style={styles.modalBackdrop}><View style={styles.chartCard}>
          <View style={styles.chartHeader}><View><Text style={styles.chartTitle}>{t("product.sizeChartTitle")}</Text><Text style={styles.chartSubtitle}>{t("product.sizeChartHelp")}</Text></View><TouchableOpacity onPress={() => setShowSizeChart(false)} style={styles.closeButton}><Ionicons name="close" size={24} color="#002041" /></TouchableOpacity></View>
          <View style={[styles.chartRow, styles.chartLabels]}><Text style={styles.chartCell}>{t("product.size")}</Text><Text style={styles.chartCell}>{t("product.height")}</Text><Text style={styles.chartCell}>{t("product.weight")}</Text></View>
