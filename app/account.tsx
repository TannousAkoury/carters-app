      ...
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        {customer ? <>
          ...
          <View style={styles.tabs}>
            {(['profile', 'addresses', 'orders'] as const).map((item) => <TouchableOpacity key={item} style={[styles.tab, section === item && styles.tabActive]} onPress={() => { setSection(item); clearNotice(); }}><Text style={[styles.tabText, section === item && styles.tabTextActive]}>{item[0].toUpperCase() + item.slice(1)}</Text></TouchableOpacity>)}
          </View>
          ...
          {section === 'orders' ? <View style={styles.section}>
            {customer.orders.edges.length ? customer.orders.edges.map(({ node }) => <View key={node.id} style={styles.card}>
              <View style={styles.cardTitleRow}><Text style={styles.cardTitle}>{node.name}</Text><Text style={styles.orderPrice}>{formatMoney(node.currentTotalPrice)}</Text></View>
              <Text style={styles.cardCopy}>{new Date(node.processedAt).toLocaleDateString()} · {prettyStatus(node.fulfillmentStatus)}</Text>
              {node.lineItems.edges.map(({ node: line }, index) => <Text key={`${line.title}-${index}`} style={styles.lineItem}>{line.quantity} × {line.title}</Text>)}
            </View>) : <Text style={styles.empty}>You have not placed an order yet.</Text>}
          </View> : null}
...
const styles = StyleSheet.create({
  ...
  card: { borderWidth: 1, borderColor: '#dfe6ec', borderRadius: 10, padding: 15, marginBottom: 12 }, cardTitleRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10 }, cardTitle: { flex: 1, color: '#002041', fontSize: 16, fontWeight: '900' }, cardCopy: { color: '#657083', lineHeight: 21, marginTop: 7 }, badge: { color: '#287a54', backgroundColor: '#e8f6ee', fontSize: 10, fontWeight: '900', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10 }, actionRow: { flexDirection: 'row', gap: 22, marginTop: 12 }, action: { color: '#006eb8', fontWeight: '800' }, orderPrice: { color: '#002041', fontWeight: '900' }, lineItem: { color: '#29384e', marginTop: 8 }, empty: { color: '#657083', textAlign: 'center', marginVertical: 25 }, formBox: { backgroundColor: '#f7f9fa', borderRadius: 10, padding: 14, marginTop: 4 },
});
